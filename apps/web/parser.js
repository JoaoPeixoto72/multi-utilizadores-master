...
try {
    ...
    for (const d of data) {
        if (d.diagnostics) {
            allDiags.push(...d.diagnostics.filter(x => x.severity === 1 || x.severity === 'error').map(x => ({ file: d.filename, msg: x.message })));
        }
    }
    ...
    Object.entries(msgCounts).sort((a, b) => b[1] - a[1]).slice(0, 10).forEach(x => console.log(x[1] + 'x: ' + x[0]));
