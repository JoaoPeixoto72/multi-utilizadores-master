...
export const load: PageServerLoad = async ({ platform, request }) => {
  ...
  const configData = res.ok
    ? (JSON.parse(bodyText) as { config: Record<string, string> })
    : { config: {} };
  ...
  const publicVars = [
    ...
    },
    {
      ...
      value: env?.APP_URL ?? "—",
      secret: false,
      description: "URL pública da aplicação (usado nos emails e links)",
    },
  ];
  ...
  const secretVars = [
    {
      key: "CSRF_SECRET",
      label: "CSRF Secret",
      configured: !!env?.CSRF_SECRET,
      description: "Segredo para tokens CSRF (min. 64 hex chars)",
    },
    {
      key: "SESSION_SECRET",
      label: "Session Secret",
      configured: !!env?.SESSION_SECRET,
      description: "Segredo para assinar sessões (min. 64 hex chars)",
    },
    {
      key: "ENCRYPTION_KEY",
      label: "Encryption Key",
      configured: !!env?.ENCRYPTION_KEY,
      description: "Chave AES-256-GCM para cifrar credenciais de integrações",
    },
    {
      key: "CF_ACCOUNT_ID",
      label: "CF Account ID",
      configured: !!env?.CF_ACCOUNT_ID,
      description: "Cloudflare Account ID (usado para PDF via Browser Rendering)",
    },
    {
      key: "CF_API_TOKEN",
      label: "CF API Token",
      configured: !!env?.CF_API_TOKEN,
      description: "Cloudflare API Token com permissão Browser Rendering",
    },
    {
      key: "SENTRY_DSN",
      label: "Sentry DSN",
      configured: !!env?.SENTRY_DSN,
      description: "DSN do Sentry para error monitoring (opcional)",
    },
  ];
  ...
  const cloudflareServices = [
    {
      ...
      binding: "DB",
      active: true,
      description: "Base de dados SQLite distribuída globalmente",
    },
    {
      label: "R2 Storage",
      binding: "R2_BUCKET",
      active: true,
      description: "Armazenamento de objectos (backups, ficheiros)",
    },
    {
      label: "Rate Limiter",
      binding: "RATE_LIMITER",
      active: true,
      description: "Durable Object para rate limiting por IP",
    },
    {
      label: "Backup Queue",
      binding: "BACKUP_QUEUE",
      active: false,
      description: "Queue de backups (requer Workers Paid plan)",
    },
  ];
